...
def main():
    ...
    parser.add_argument("--excel", required=False, help="Absolute path to Excel/CSV file containing payslip data (single-file mode)")

    parser.add_argument("--employee-excel", required=False, help="Absolute path to Excel file containing employee details (sheet 'Employee Information' by default)")

    parser.add_argument("--employee-sheet", required=False, default="Employee Information", help="Sheet name for employee details (default: 'Employee Information')")

    parser.add_argument("--salary-excel", required=False, help="Absolute path to Excel file containing salary details (sheet 'sheet0' by default)")

    parser.add_argument("--salary-sheet", required=False, default="sheet0", help="Sheet name for salary details (default: 'sheet0')")
    ...
    # Data loading: two-file mode or single-file mode
    df: pd.DataFrame
    if args.employee_excel and args.salary_excel:
        if not os.path.isabs(args.employee_excel):
            raise SystemExit("--employee-excel must be an absolute path")
        if not os.path.isabs(args.salary_excel):
            raise SystemExit("--salary-excel must be an absolute path")
        df = prepare_two_file_merged_dataframe(
            employee_excel_path=args.employee_excel,
            salary_excel_path=args.salary_excel,
            employee_sheet_name=args.employee_sheet,
            salary_sheet_name=args.salary_sheet,
        )
    else:
        if not args.excel:
            raise SystemExit("Provide either --excel (single-file mode) or both --employee-excel and --salary-excel (two-file mode)")
